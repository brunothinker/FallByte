...
def safe_cleanup_video_session_files(
    created_files: Iterable[Path],
    root_output_dir: Path
) -> int:
    """
    Safely removes video files created during a cancelled processing session.

    Ensures that root_output_dir is NEVER deleted, preserving the user's pre-existing files.
    Purges only empty subdirectories created during the current video session.
    ...
    Returns:
        int: Total number of video files successfully deleted during cleanup.
    """
    cleaned_count = 0
    affected_dirs = set()

    # Delete session video files and collect their parent subdirectories
    for dst_file in created_files:
        if dst_file.exists():
            try:
                ...
                logger.warning(f"Failed to remove session video file '{dst_file}': {e}")
